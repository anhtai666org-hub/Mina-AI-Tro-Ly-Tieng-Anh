...
<head>
    ...
    <title>English Assistant AI</title>
    <style>
        :root {
...
<body>
    ...
    <div class="vtuber-top-right">
        <div class="avatar-box" onclick="document.getElementById('avatarInput').click()" title="Đổi Avatar">
            <img id="minaAvatar" src="https://api.iconify.design/fluent-emoji:books.svg" alt="Avatar">
        </div>
        <input type="file" id="avatarInput" accept="image/*" style="display:none" onchange="loadAvatar(event)">
        <div class="vtuber-info">
            <span class="vtuber-name">Mina AI 📚</span>
    ...
    <div class="chat-container">
        <div class="chat-box" id="chatBox">
            <div class="message ai-message">Hello đại vương! Tớ là Mina - trợ lý tiếng Anh của cậu đây ❤️ Cậu cần giải nghĩa từ vựng hay luyện tập gì nào?</div>
        </div>
        ...
        <div class="input-box">
            ...
            <input type="text" id="userInput" placeholder="Hỏi gì về tiếng Anh đi bé iu❤️..." onkeypress="checkEnter(event)">
    ...
    <div id="imageModal" class="img-modal" onclick="closeImageModal()">
        ...
        <div class="img-modal-actions" onclick="event.stopPropagation()">
            <a id="downloadImgBtn" download="mina_english_image.png" class="btn-download-img">💾 Tải ảnh về</a>
    ...
    <script>
        ...
        function loadChat(chatId) {
            ...
            if (hist.length === 0) {
                chatBox.innerHTML = '<div class="message ai-message">Hello đại vương! Tớ là Mina - trợ lý tiếng Anh của cậu đây ❤️ Cậu cần giải nghĩa từ vựng hay luyện tập gì nào?</div>';
            } else {
                hist.forEach(item => {
        ...
        async function sendMessage() {
            const inputField = document.getElementById('userInput'), text = inputField.value.trim();
            ...
            try {
                ...
                const response = await fetch('/chat', {
                    ...
                    body: JSON.stringify({ message: text, image: sentImg, history: cleanHistory })
                });
...
def chat():
    ...
    # Lấy API Key trực tiếp từ biến môi trường của Render (hoặc hệ thống)
    user_api_key = os.environ.get("GEMINI_API_KEY", "")
    if not user_api_key:
        return jsonify({"type": "text", "reply": "Chưa cấu hình GEMINI_API_KEY trên biến môi trường của Render!"})
    ...
    payload = {
        "system_instruction": {"parts": [{"text": "Mày tên là Mina, là một AI Trợ lý Tiếng Anh thông minh và thân thiện. Mày chuyên hỗ trợ học sinh học từ vựng, giải nghĩa cấu trúc câu, sửa lỗi ngữ pháp và luyện tập tiếng Anh. Mày xưng là Mina và nói chuyện nhí nhảnh, dễ thương, truyền cảm hứng học tập."}]},
